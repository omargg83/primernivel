...
/*********************************reporte_remesa.php**************************************/
	...
	public function rep_remesa_sede($id){
	...
	public function sedes_activas_remesa($id){
		$sql="select `cat_sedes`.`id_sede`,`cat_sedes`.`nombre_sede`, `cat_sedes`.`id_juris`,`cat_jurisdicciones`.`nombre_juris`,`cat_municipios`.`nombre_mpio`
					...
					where `sede_activa`=1 and `id_remesa`='$id' ORDER BY `cat_jurisdicciones`.`nombre_juris` asc, `cat_municipios`.`nombre_mpio`";
	$sth = $this->dbh->query($sql);
	return $sth->fetchAll(PDO::FETCH_OBJ);
	}
/*********************************lista_fechas.php**************************************/
					...
					public function rep_fecha_sede($fechar,$tabla,$id){
						if($tabla==1){
								$sql="select sum(`H_60`+`H_65`+`H_70`+`H_80`+`H_75`+`M_60`+`M_65`+`M_70`+`M_80`+`M_75`) AS 'total_vac_pob',
										sum(`briga`) AS 'total_brig',
										sum(`personal_salud`) AS 'total_per_sal',
										sum(`H_60`+`H_65`+`H_70`+`H_80`+`H_75`+`M_60`+`M_65`+`M_70`+`M_80`+`M_75`+`briga`+`personal_salud`) AS 'total_dosis',
										sum(`HENG_60`+`HENG_65`+`HENG_70`+`HENG_80`+`HENG_75`+`MENG_60`+`MENG_65`+`MENG_70`+`MENG_80`+`MENG_75`) AS 'total_ENG',
										sum(`HEG_60`+`HEG_65`+`HEG_70`+`HEG_80`+`HEG_75`+`MEG_60`+`MEG_65`+`MEG_70`+`MEG_80`+`MEG_75`) AS 'total_EG',
										sum(`merma`) AS 'total_mermas'
										from `det_vac_60ymas`
